    ...
    modules: {
      toolbar: {
        ...
        handlers: {
          ...
          clear: function () { if (confirm('Clear all content?')) { if (E.writerTitleInput) E.writerTitleInput.value = ''; quill.setText(''); updateWriterWordCount(); } },
        },
      },
      keyboard: true,
...
function showLinkDialog() {
  ...
  overlay.innerHTML = `
    <div class="modal-card" style="max-width:420px">
      <h2 style="margin-bottom:12px">${hasSelection ? "Link selected text" : "Insert link"}</h2>
      <input type="text" id="wrLinkUrl" placeholder="Paste URL…" style="width:100%;margin-bottom:12px">
      <div class="btn-row">
        <button class="btn-primary" id="wrLinkInsert">${hasSelection ? "Link" : "Insert"}</button>
        <button class="btn-secondary" id="wrLinkCancel">Cancel</button>
      </div>
    </div>
  `;
  document.body.appendChild(overlay);
  overlay.querySelector("#wrLinkCancel").onclick = () => overlay.remove();
  overlay.querySelector("#wrLinkInsert").onclick = () => {
    const url = overlay.querySelector("#wrLinkUrl").value.trim();
    if (url && quill) {
      if (hasSelection) {
        quill.format('link', url, 'user');
      } else {
        const r = quill.getSelection(true);
        quill.insertText(r.index, url, 'link', url, 'user');
        quill.setSelection(r.index + url.length, 0);
      }
    }
    overlay.remove();
  };
  overlay.querySelector("#wrLinkUrl").focus();
  overlay.addEventListener("click", e => { if (e.target === overlay) overlay.remove(); });
}
  ...
  list.innerHTML = drafts.map(d => `
    <div class="wr-draft-card" data-draft-id="${d.id}">
      ...
      <div class="wr-draft-actions">
        ...
        <button class="wr-draft-btn" data-draft-save="${d.id}" title="Save current content here">🗃</button>
...
function saveToDraft(id) {
  ...
  if (!d) return;
  d.content = content;
  d.title = (E.writerTitleInput?.value || "").trim() || "Untitled";
...
export function initDraftLibrary() {
  ...
  E.writerDraftsList?.addEventListener("click", e => {
    ...
    const saveBtn = e.target.closest("[data-draft-save]");
    ...
    if (saveBtn) { saveToDraft(saveBtn.dataset.draftSave); return; }
...
function showMentionPopup(results, query) {
  ...
  const editorRect = E.writerEditor.getBoundingClientRect();
  ...
  let top = editorRect.top + pos.bottom + 4;
...
function handleMentionTrigger() {
  ...
  if (atPos !== -1 && cursorPos - atPos <= 30 && cursorPos > atPos) {
    const rawQuery = text.slice(atPos + 1, cursorPos);
...
async function handleQuillPaste(e) {
  try {
    ...
    if (urlRegex.test(text)) pendingPasteUrlCheck = true;
  } catch {}
}
...
async function resolvePastedUrls() {
  ...
  try {
    ...
    while ((match = re.exec(content)) !== null) {
      found.push({ full: match[0], type: match[1], id: match[2], index: match.index, name: mentionCache.get(match[1] + ':' + match[2]) });
    }
    ...
    for (let i = found.length - 1; i >= 0; i--) {
      ...
      if (!f.name) {
        try {
          ...
          f.name = entityDisplayName(f.type, f.id, data);
          if (f.name) mentionCache.set(f.type + ':' + f.id, f.name);
        } catch { continue; }
      }
      const curPos = quill.getText().indexOf(f.full, Math.max(0, f.index - 5));
      if (curPos === -1) continue;
      f.index = curPos;
      quill.deleteText(f.index, f.full.length, 'user');
      const cur = quill.getText();
      const charBefore = f.index > 0 ? cur[f.index - 1] : null;
      const charAfter = cur[f.index];
      if (charBefore && charBefore !== ' ' && charBefore !== '\n') {
        quill.insertText(f.index, ' ', 'user');
        f.index += 1;
      }
      quill.insertEmbed(f.index, 'mention', { entityType: f.type, entityId: f.id, entityName: f.name }, 'user');
...
export function copyWriterHtml() {
  ...
  wrapDiv.querySelectorAll('[class*="ql-align-"]').forEach(el => {
    for (const cls of el.className.split(' ')) {
      const m = cls.match(/^ql-align-(.+)$/);
      if (m) { el.style.textAlign = m[1]; break; }
    }
    el.classList.remove(...Array.from(el.classList).filter(c => c.startsWith('ql-align-')));
  });
  // Add tiptap-block class to <p> elements
  wrapDiv.querySelectorAll('p').forEach(p => {
    if (!p.classList.contains('tiptap-block')) p.classList.add('tiptap-block');
  });
  // Convert <pre class="ql-syntax"> to tiptap-code-block
  wrapDiv.querySelectorAll('pre.ql-syntax').forEach(pre => {
    pre.className = 'tiptap-code-block';
    const code = document.createElement('code');
    while (pre.firstChild) code.appendChild(pre.firstChild);
    pre.appendChild(code);
  });
  // Add tiptap-image class to <img> elements
  wrapDiv.querySelectorAll('img').forEach(img => img.classList.add('tiptap-image'));
  // Add tight class and data-tight to lists
  wrapDiv.querySelectorAll('ol, ul').forEach(list => {
    list.classList.add('tight');
    list.setAttribute('data-tight', 'true');
  });
  // Clean up <hr> — remove inline styles
  wrapDiv.querySelectorAll('hr').forEach(hr => hr.removeAttribute('style'));
  // Wrap YouTube <iframe> in <div data-youtube-video="">
  wrapDiv.querySelectorAll('iframe[src*="youtube"]').forEach(iframe => {
